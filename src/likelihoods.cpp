...
double sat_int(double r, double N, double t0, double t, double delta_t){

  double out = std::numeric_limits<double>::infinity();
  if (t+t0 < 0 && t+delta_t+t0 < 0) {
    out = ((1/N)*((t0+t+delta_t)-(1/r)*(1/(t0+t+delta_t))))-((1/N)*((t0+t)-(1/r)*(1/(t0+t))));
  } 

  return out;
}

double sat_rate(double r, double N, double t0, double t){
  return (1/N)*((1+r*std::pow(t0+t,2))/(r*std::pow(t0+t,2)));
}
...
double coalescent_loglh(NumericVector sampling_times,
    ...
    while (c_idx < coalescent_times.length()) {
        ...
        if (s_idx+1 < sampling_times.length() && coalescent_times[c_idx] < sampling_times[s_idx+1]){
            ...
            ++c_idx;
            log_lh += std::log(const_rate(pop_size,t+delta_t)) + comb_n*const_int(pop_size, t, delta_t);
            --k;
            t += delta_t;
        }
    }
...
//' @export
// [[Rcpp::export]]
double sat_coalescent_loglh(NumericVector sampling_times,
                                     NumericVector coalescent_times,
                                     const double t0,
                                     const double r,
                                     const double N,
  ...
  while (c_idx < coalescent_times.length()) {
    ...
    if (s_idx+1 < sampling_times.length() && coalescent_times[c_idx] < sampling_times[s_idx+1]){
        ...
        ++s_idx;
        delta_t = t_max - t - sampling_times[s_idx];
        log_lh += comb_n*sat_int(r, N, t0, t, delta_t);
        ++k;
        t += delta_t;
    } else {
        //Coalescent
        delta_t = t_max - t - coalescent_times[c_idx];
        ++c_idx;
        log_lh += std::log(sat_rate(r, N, t0, t+delta_t)) + comb_n*sat_int(r, N, t0, t, delta_t);
        --k;
        t += delta_t;
    }
  }
