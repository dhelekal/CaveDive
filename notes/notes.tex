...
\end{gather*}
Under the assumption that $\mathbf{g}$ is a genealogy of a given sample, with each edge in $E_\mathbf{g}$ there is an associated unobserved set of individuals descending from one another. At some point along an edge a node of one lineage will undergo colour change and change its type to that of any another extant lineage.\\
First, we define a coalescent model with multiple lineages.
...
\begin{figure}[H]
  \centering
  \subfloat[]{\label{trace:a}\includegraphics[scale=.25]{../R/test_mcmc/trace}}
  \centering
  \subfloat[]{\label{trace:b}\includegraphics[scale=.25]{../R/test_mcmc/trace_zoom}}
  \centering
...
\begin{figure}[H]
  \centering
  \subfloat[]{\label{hists:a}\includegraphics[scale=.4]{../R/test_mcmc/lambdahist}}
  \centering
  \subfloat[]{\label{hists:b}\includegraphics[scale=.4]{../R/test_mcmc/nhist}}
  \centering
