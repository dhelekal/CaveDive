...
\mathcal{L}(T(\xi_j) \mid \alpha_j) &= \prod_{t_i\in T(\xi_j)}\left(\mathbbm{1}_{t_Y\cup\tau_{W(\xi_j)}}(t_i) \frac{\binom{k_{i,\xi_j}}{2}}{\alpha(t_i)}+\mathbbm{1}_{t_I}(t_i)\right)
...
\begin{gather}
I(t) = \frac{1-\frac{1}{R_0}N}{1+\frac{1}{I_0}(1-\frac{1}{R_0}N)\exp((\beta-\gamma)t)}
\end{gather}
with $R_0 = \beta/\gamma$\\
Setting $\alpha(t) = I(t)$
\begin{gather}
\int_t^{t+s}{\alpha^{-1}(\tau)\,d\tau} = \left[\frac{\tau}{1-\frac{1}{R_0}N} + \frac{1}{(I_0)(\beta-\gamma)}\exp((\beta-\gamma)t)\right]_t^{t+s}  
\end{gather}
As our hypothesis posutlates that a divergence event is a new lineage being born and undergoing rapid growth, $I_0$ starts near zero, and is a fixed hyperparameter. This leaves us with a population size function with two free parameters
