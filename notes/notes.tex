...
Given $M$ colours, $M$ population size functions $\mathbf{Neg}\triangleq\{Neg_j(t)\}_{1\leq j\leq M}$. Let $Y(t)$ be a CTMC with the state space:
\begin{gather}
...
Assume that we have been given a genealogy as described in previous section. The number of individua lineages $M$, the effective population size functions $\mathbf{Neg}$, the placement and divergence events in descending order $\Xi =\{\xi_i\}_{1\leq i\leq M}$, are unknown. In effect we seek to identify terms in the factorisation of the posterior:
\begin{gather}
\mathcal{L}(\mathbf{Neg}, \Xi, M\mid\mathbf{g}) \propto 
\mathcal{L}(\mathbf{g}\mid \Xi, M, \mathbf{Neg})\mathcal{L}(\mathbf{Neg}\mid\Xi,M)\mathcal{L}(\Xi,M)
\end{gather}
...
\end{definition}
In order to identify the likelihood $\mathcal{L}(\mathbf{g}\mid \Xi, M, \mathbf{Neg})$, the genealogy needs to be first partitinioned into separate subtrees, each corresponding to an individual lineage.
...
\begin{gather}
W(\xi_i) \triangleq v^{MRCA}_i\cup\mathcal{D}_(v^{pa}_i) \setminus \bigcup_{j<i}(\mathcal{D}(v^{pa}_j))
\end{gather} 
\end{definition}
...
\begin{gather}
\begin{aligned}
\mathcal{L}(\mathbf{g}\mid \Xi, M, \mathbf{Neg}) &= \prod_{j=1}^{M}\mathcal{L}(T(\xi_j) \mid Neg_j) \\
\mathcal{L}(T(\xi_j) \mid Neg_j) &= \prod_{t_i\in T(\xi_j)}\left(\mathbbm{1}_{t_Y\cup\tau_{W(\xi_j)}}(t_i) \frac{\binom{k_{i,\xi_j}}{2}}{Neg(t_i)}+\mathbbm{1}_{t_I}(t_i)\right)
