...
The framework described is being implemented as an R package, featuring the ability to simulate genealogies under the model we propose, compute model likelihoods for varying parameters. and to perform inference under same model using MCMC. 
...
\end{figure}
\subsubsection{MCMC inference}
At the moment we are in the process of implementing, and valdiating the MCMC routine for inference under coalescent with local population structure. We present the following intermediate results, however it is important to note that the proposal distributions are still pending validation and rigorous testing.\\
For the example, a phylogeny with 100 sampling events and a single diverging subpopulation has been simulated. The location of the divergence event on the tree, the growth rate and carrying capacity of the diverging subpopulation, as well as the neutral population size of the parent population have then been inferred using MCMC run for one million iterations. 
