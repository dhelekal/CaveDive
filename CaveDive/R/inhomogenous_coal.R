inhomogenous_coal.simulate <- function(sampling_times,
  ...
  while (future_lineages > 0 || extant_lineages > 1) {
    if (extant_lineages < 2) {
      ...
      p_coal <- inhomogenous_exp.prob(c * Neg.rate.int, t, s)
      r <- runif(1, 0, 1)
      
      if (r <= p_coal) {
        r_w <- runif(1, 0, 1)
        w_t <- inv_t_inhomogenous_exp_conditional(Neg.rate.int_inv,
                                                  Neg.rate.int,
                                                  c, t, s)
        
        log_lh <- log_lh + inhomogenous_exp.lh(Neg.rate,
                                               Neg.rate.int,
                                               t, w_t)
...
inhomogenous_coal.log_lh <- function(sampling_times,
  ...
  while (coal_idx <=  n_coal) {
    if (sample_idx < n_sample &&
      ...
      s <- times_desc[sample_idx]
      rate.int <- Neg.int*choose(j, 2)
      ...
      rate.int <- Neg.int*choose(j,2)
      rate <- Neg*choose(j,2)
      log_lh <- log_lh + log(inhomogenous_exp.lh(rate.int, rate, t,s))
