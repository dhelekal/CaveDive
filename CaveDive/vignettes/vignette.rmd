...
We shall use `phy2` to demonstrate performing inference under the CaveDive model. Firstly, it is preferable to use less informative priors for inference when little information is available. 
...
n_it <- 2e6
thinning <- n_it/1e4
phy_inf <- collapse.singles(phy2$tree)
start <- Sys.time()
expansions <- run_expansion_inference(phy_inf, inference_priors, concentration=1, n_it=n_it, thinning=thinning) 
end <- Sys.time()
...
```{r plot summary, out.width="90%", fig.width=10, fig.height=10, dpi=300, fig.align="center", message=FALSE,eval=T} 
