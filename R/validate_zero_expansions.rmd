...
rm -rf ./Paper/Sim/tmp2

mkdir ./Paper/Sim/tmp2
```
...
find ./Paper/Sim/sim_zero_exp | grep .*.txt | xargs -L 1 cat | parallel -j 10 --verbose --tmpdir ./Paper/Sim/tmp2 -n 8 --halt-on-error 2 eval Rscript "./Scripts/simulate_tree.R {}"
```

### Run MCMC inference 
```{bash, run mcmc, results=FALSE, eval=eval_mcmc}
find ./Paper/Sim/sim_zero_exp | grep .*tree.nwk | xargs dirname | parallel -j 10 --verbose --tmpdir ./Paper/Sim/tmp2 --halt-on-error 2 eval Rscript "./Scripts/run_expansions_inference_nwk.R -n 2e6 -t 2e2 --lambdar 1 -s 1 -f {}/tree.nwk -o {}"
```
...
prop_correct <- length(which(data_df$mode_dim==0))/length(data_df$mode_dim)
print(paste("Proportion Correct:", prop_correct))
print(paste("Incorrect:", experiment_dirs[which(data_df$mode_dim>0)]))
```
