...
dir.create(file.path(".", "sim_zero_exp"))
base_dir <- "./sim_zero_exp"
```
...
find ./sim_zero_exp | grep .*.txt | xargs -L 1 cat | parallel -j 20 --verbose --tmpdir ./tmp -n 8 --halt-on-error 2 eval Rscript "../../R/Scripts/simulate_tree.R {}"
```

### Run MCMC inference 
```{bash, run mcmc, results=FALSE, eval=eval_mcmc}
find ./sim_zero_exp | grep .*tree.nwk | xargs dirname | parallel -j 20 --verbose --tmpdir ./tmp --halt-on-error 2 eval Rscript "../../R/Scripts/run_expansions_inference_nwk.R -n 2e6 -t 2e2 --lambdar 1 -s 1 -f {}/tree.nwk -o {}"
```

### Produce plots for dimension mode analysis
```{r, process data}
experiment_specs <- list.files(path = "./sim_zero_exp/test_out", pattern=".*tree_params.json", full.names = TRUE, recursive = TRUE)
...
results_data <- lapply(c(1:length(experiment_specs)), function(i){
  ...
  if(length(correct_dim_it) > 0) {
    exp_N <- sum(correct_dim$N)/(length(correct_dim$N))
  } else {
    exp_N <- NA
  }
