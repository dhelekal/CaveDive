...
dir.create(file.path(".", "sim_one_exp"))
base_dir <- "./sim_one_exp"
```
...
find ./sim_one_exp | grep .*.txt | xargs -L 1 cat | parallel -j 20 --verbose --tmpdir ./tmp -n 10 --halt-on-error 2 eval Rscript "../../R/Scripts/simulate_tree.R {}"
```

### Run MCMC inference 
```{bash, run mcmc, results=FALSE, eval=eval_mcmc}
find ./sim_one_exp | grep .*tree.nwk | xargs dirname | parallel -j 20 --verbose --tmpdir ./tmp --halt-on-error 2 eval Rscript "../../R/Scripts/run_expansions_inference_nwk.R -n 2e6 -t 2e2 --lambdar 5 -s 1 -f {}/tree.nwk -o {}"
```
...
experiment_specs <- list.files(path = "./sim_one_exp/test_out", pattern=".*tree_params.json", full.names = TRUE, recursive = TRUE)
...
results_data <- lapply(c(1:length(experiment_specs)), function(i){
  ...
  tree <- read.tree(file = paste0(experiment_dirs[i],"/tree.nwk")) 
  pre <- structured_coal.preprocess_phylo(tree, order_edges_by_node_label=FALSE) 
  ...
  mcmc_data <- read.csv(paste0(experiment_dirs[i],"/mcmc_data.csv"))
  mcmc_data <- mcmc_data[burn_in:nrow(mcmc_data),]
  event_data <- read.csv(paste0(experiment_dirs[i],"/event_data.csv"))
  event_data <- event_data[which(event_data$it >= burn_in), ]
...
png("dim_summary.png", width=1600,height=800)
p
dev.off()
p
```

```{r, plot param panel, eval=T}
...
N_scatter <- N_scatter + xlim(lims) + ylim(lims)
...
K_scatter <- K_scatter + xlim(lims) + ylim(lims)
...
T_scatter <- T_scatter + xlim(lims) + ylim(lims)
...
t_mid_scatter <- t_mid_scatter + xlim(lims) + ylim(lims)

p <- ggarrange(N_scatter, K_scatter, t_mid_scatter, T_scatter, widths=c(2,2), heights=c(2,2))
png("param_summary.png", width=1600,height=1600)
p
dev.off()
p
