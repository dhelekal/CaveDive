...
find ./Paper/Sim/sim_one_exp | grep .*.txt | xargs -L 1 cat | parallel -j 20 --verbose --tmpdir ./Paper/Sim/tmp -n 10 --halt-on-error 2 eval Rscript "./Scripts/simulate_tree.R {}"
```

### Run MCMC inference 
```{bash, run mcmc, results=FALSE, eval=eval_mcmc}
find ./Paper/Sim/sim_one_exp | grep .*tree.nwk | xargs dirname | parallel -j 20 --verbose --tmpdir ./Paper/Sim/tmp --halt-on-error 2 eval Rscript "./Scripts/run_expansions_inference_nwk.R -n 1e7 -t 1e3 --lambdar 5 -s 1 -f {}/tree.nwk -o {}"
```
