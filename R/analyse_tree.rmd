...
```{bash, clean directory, include=FALSE, eval=T}
rm -rf ./mcmc_out
mkdir mcmc_out
```
...
```{r, extract marginal}
  mode_dim_marginal <- mcmc.df[which(mcmc.df$dim==dim.gt),]
...
```{r, fig.cap="Tree Branch Frequency. Left: Marginal. Right: Full posterior", out.width="50%", fig.hold=T, echo=F}
marginal_freq <- plot_tree_freq(mcmc.df, event.df, pre, prior_t_given_N=function(t,N) exp(priors$prior_t_given_N(t,N)))
full_freq <- plot_tree_freq(mode_dim_marginal.df, event_mode_dim_marginal.df, pre, prior_t_given_N=function(t,N) exp(priors$prior_t_given_N(t,N)))
plot(marginal_freq)
plot(full_freq)
```