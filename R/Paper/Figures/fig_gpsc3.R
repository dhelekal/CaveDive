...
base_dir <- "../GPSC_Trees/GPSC3"
expansions <- readRDS(file = paste0(base_dir, "/mcmc_out/expansions.rds"))
expansions <- discard_burn_in(expansions, proportion=burn_in)

corr_df <- read.csv(paste0(base_dir, "/microreact-project-gpsGPSC3-data.csv"),row.names=1)
corr_df <- as.data.frame(corr_df)

corr_df1 <- corr_df[,c("PBP1A_2B_2X__autocolour"),drop=F]
corr_df2 <- corr_df[,"Country",drop=F]
...
png("fig_gpsc3_corr.png", width=1600, height=1600)
plot(expansions, mode="persistence", k_modes=3, correlates=list(corr_df1, corr_df2))
dev.off()

png("fig_gpsc3_param.png", width=1600, height=1600)
