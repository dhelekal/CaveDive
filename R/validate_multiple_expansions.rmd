...
e_hist <- ggplot(hist_df) + 
          geom_bar(aes(y=factor(dim,levels=c(0:20), ordered=T), x = (..count..)/sum(..count..))) + 
          scale_y_discrete(limits=c(0:9)) + 
          coord_cartesian(ylim = c(-0.5, 9.5), xlim=c(0,0.15),expand = FALSE) +
...
e_dim_scatter <- e_dim_scatter + geom_boxplot(aes(group=dim_gt))
